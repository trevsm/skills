...
- **[angles](./angles/SKILL.md)** — Works a hard question as a graph of smaller questions, settled bottom-up like a careful person reasons: a framer pins down terms, category errors, and the crux, then pieces are settled deepest first, each building on the settled answers below it. Pieces can ask for a missing dependency (a matcher reuses an existing piece when it is the same question), send a weak one back, or say they are framed wrong. Cheap models settle facts; a strong model frames, weighs, writes, and challenges. A Python script owns the graph, the dollar budget, and every prompt, so the planner is a dumb loop. Triggered by saying *"angles"* or *"every angle"*.
