...
- **[angles](./angles/SKILL.md)** — Works a question or goal as a planner-led tree of agents. The planner sets checkable done-when criteria, sends waves of branch leads that run their own workers, folds their reports back in, and picks the next wave until the criteria are met or the depth and agent caps stop it. A challenger then attacks the draft before synthesis. A Python ledger script owns the budget, caps, prompts, and stop rules so the planner cannot skip steps. Triggered by saying *"angles"* or *"every angle"*.
